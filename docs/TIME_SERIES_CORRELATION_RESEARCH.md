...
> 状态：Phase 2 稳健性审计已完成；下一步为预注册的独立崩盘/对照案例验证
> 日期：2026-08-18
...
Phase 2 已完成 uPEG 与 TURBO 的移动区块 bootstrap、区块置换检验、
Benjamini-Hochberg 多重检验校正和多时间桶敏感性。uPEG 的价格收益与目标代币
储备变化在 1/2/4/6 小时桶内保持稳健同期负相关，但全部预测性非零 lag 均未
通过校正。TURBO 仅机械性的“储备变化 vs 净 LP 流”通过同期家族校正；此前
成交量领先价格/累计撤资的候选均未通过 lead-lag 家族校正。完整证据与解释边界见
`research-notes/correlation-robustness-audit.md`。
